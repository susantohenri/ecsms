...
function formInit(scope) {
    ...
    scope.find('select').not('.select2-hidden-accessible').not('[name="pemenang"]').each(function () {
...
function select2pemenang() {
    if ($('[name="pemenang"]').is('[data-autocomplete]')) {
        var model = $('[name="pemenang"]').attr('data-model')
        var field = $('[name="pemenang"]').attr('data-field')
        $('[name="pemenang"]').select2({
            ajax: {
                url: current_controller_url + '/select2/' + model + '/' + field,
                type: 'POST', dataType: 'json'
                , success: function (data) {
                    var peserta = []
                    $('[name*="PesertaProject_vendor"]').each(function () {
                        let uuid = $(this).find('option:selected').attr('value')
                        peserta.push(uuid)
                    })
                    data.results = data.results.filter(function (result) {
                      return peserta.indexOf(result.id) > -1
                    })
                    return data
                }
            }
